...
|------|---------|----------|
| Generic skill | `{technology}` | `vitest`, `playwright`, `typescript` |
| gero-specific | `gero-{component}` | `gero-api`, `gero-ui`, `gero-sdk-check` |
| Testing skill | `gero-test-{component}` | `gero-test-sdk`, `gero-test-api` |
...
## Decision: Gero-Specific vs Generic

```
Patterns apply to ANY project?     → Generic skill (e.g., pytest, typescript)
Patterns are Gero-specific?     → gero-{name} skill
Generic skill needs Gero info?  → Add references/ pointing to Gero docs
```
...
| `license` | Yes | Always `Apache-2.0` for Gero |
| `metadata.author` | Yes | `gero-cloud` |
