...
import { MedicationSchema } from '@/business/medication/domain/Medication.schema'
...
} from '@/business/medication/domain/MedicationErrors'
 ...
 */
function firestoreDocToMedication(
	docId: string,
	...
	try {
		...
		if (!result.success) {
			...
			return Err(
				createMedicationValidationError(
					`Invalid medication data from Firestore: ${firstError?.message || 'Validation failed'}`
				)
			)
		}
		...
		const appError = toAppError(error, 'Error al convertir documento de Firestore a Medication')
		logError(appError, { docId, operation: 'firestoreDocToMedication' })
		return Err(createUnknownMedicationError(appError.message))
	}
}
 ...
 */
function firestoreDocToAdministration(
	docId: string,
	data: Record<string, unknown>
...
export function createMedicationRepository(db: Firestore): MedicationRepository {
	...
	async function create(
		...
		try {
			...
			const appError = toAppError(error, 'Error al crear medicación')
			logError(appError, { operation: 'create', medication })
			return Err(createUnknownMedicationError(appError.message))
		}
	}
	...
	async function findById(id: string): Promise<Result<Medication | null, MedicationError>> {
		try {
			...
			const appError = toAppError(error, 'Error al buscar medicación')
			logError(appError, { operation: 'findById', id })
			return Err(createUnknownMedicationError(appError.message))
		}
	}

	async function findByResident(
		residentId: string
	): Promise<Result<Medication[], MedicationError>> {
		try {
			const q = query(
			...
			const medicationResults = querySnapshot.docs.map(doc =>
				firestoreDocToMedication(doc.id, doc.data())
			)
			...
			const medications = medicationResults
				.map(r => (r.success ? r.value : null))
				.filter((m): m is Medication => m !== null)
			return Ok(medications)
		} catch (error) {
			const appError = toAppError(error, 'Error al buscar medicaciones por residente')
	...
	async function findAll(): Promise<Result<Medication[], MedicationError>> {
		try {
			...
			const appError = toAppError(error, 'Error al buscar todas las medicaciones')
	...
	async function update(
		...
		try {
			...
			if (error && typeof error === 'object' && 'code' in error && error.code === 'not-found') {
				return Err(createMedicationNotFoundError(`Medicación con id ${id} no encontrada`))
			}
			const appError = toAppError(error, 'Error al actualizar medicación')
	...
	async function deleteMedication(id: string): Promise<Result<void, MedicationError>> {
		try {
			...
			const appError = toAppError(error, 'Error al eliminar medicación')
	...
	async function recordAdministration(
		...
		try {
			...
			const appError = toAppError(error, 'Error al registrar administración de medicación')
			logError(appError, { operation: 'recordAdministration', medicationId, administration })
			return Err(createUnknownMedicationError(appError.message))
		}
	}

	async function getAdministrationHistory(
		medicationId: string
	): Promise<Result<MedicationAdministration[], MedicationError>> {
		try {
			const q = query(
			...
			const querySnapshot = await getDocs(q)
			const administrations = querySnapshot.docs.map(doc =>
				firestoreDocToAdministration(doc.id, doc.data())
			)
			return Ok(administrations)
		} catch (error) {
			const appError = toAppError(error, 'Error al obtener historial de administraciones')
			logError(appError, { operation: 'getAdministrationHistory', medicationId })
			return Err(createUnknownMedicationError(appError.message))
		}
	}

	async function getResidentAdministrationHistory(
		residentId: string
	): Promise<Result<MedicationAdministration[], MedicationError>> {
		try {
			const q = query(
			...
			const appError = toAppError(
				error,
				'Error al obtener historial de administraciones por residente'
			)
			logError(appError, { operation: 'getResidentAdministrationHistory', residentId })
			return Err(createUnknownMedicationError(appError.message))
		}
	}
