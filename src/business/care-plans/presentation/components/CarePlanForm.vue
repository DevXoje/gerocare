  ...
  >
    <form class="care-plan-form" @submit.prevent="handleSubmit">
      <FormField v-if="!residentId" label="Residente ID" required>
        <Input :model-value="form.residentId || ''" @update:model-value="(v) => form.residentId = v" type="text" required placeholder="ID del residente" :error="error || undefined" />
      </FormField>

      <FormField label="Título" required>
        <Input :model-value="form.title || ''" @update:model-value="(v) => form.title = v" type="text" required placeholder="Ej: Higiene diaria" />
      </FormField>

      <FormField label="Descripción" required>
        <Textarea :model-value="form.description || ''" @update:model-value="(v) => form.description = v" :rows="3" required placeholder="Descripción detallada del plan de atención" />
      </FormField>

      <FormField label="Categoría" required>
        <Select :model-value="form.category || 'hygiene'" @update:model-value="(v) => form.category = String(v)" :options="categories" required />
      </FormField>

      <FormField label="Frecuencia" required>
        <Select :model-value="form.frequency || 'daily'" @update:model-value="(v) => form.frequency = String(v)" :options="frequencies" required />
      </FormField>

      <FormField label="Prioridad" required>
        <Select :model-value="form.priority || 'medium'" @update:model-value="(v) => form.priority = v" :options="priorities" required />
      </FormField>
