...
import { getCategoryDisplayName,isCarePlanActive } from '../../domain/CarePlan'
...
const activeCarePlans = computed(() => {
  return props.carePlans.filter((cp) => isCarePlanActive(cp))
})

const getPriorityVariant = (priority: CarePlan['priority']): 'default' | 'primary' | 'warning' | 'error' => {
...
const getStatusVariant = (status: CarePlan['status']): 'default' | 'primary' | 'success' | 'warning' => {
      ...
      >
        <div class="care-plan-item">
          ...
          <div class="care-plan-item__meta">
            <span class="care-plan-item__category">{{ getCategoryDisplayName(carePlan.category) }}</span>
