...
import { CarePlanSchema } from '../domain/CarePlan.schema'
 ...
 */
function firestoreDocToCarePlan(docId: string, data: Record<string, unknown>): Result<CarePlan, CarePlanError> {
  try {
    ...
    if (!result.success) {
      ...
      return Err(createCarePlanValidationError(`Invalid care plan data from Firestore: ${firstError?.message || 'Validation failed'}`))
    }

    return Ok(result.data)
  } catch (error) {
    return Err(createUnknownCarePlanError('Failed to convert Firestore document to CarePlan'))
  }
}
...
export function createCarePlanRepository(db: Firestore): CarePlanRepository {
  ...
  async function create(carePlan: Omit<CarePlan, 'id' | 'createdAt' | 'updatedAt'>): Promise<Result<CarePlan, CarePlanError>> {
    try {
      const now = new Date()
      const carePlanData = {
        ...carePlan,
      ...
      return Err(createUnknownCarePlanError(message))
    }
  }
  ...
  async function findById(id: string): Promise<Result<CarePlan | null, CarePlanError>> {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to find care plan'
      return Err(createUnknownCarePlanError(message))
    }
  }
  ...
  async function findByResident(residentId: string): Promise<Result<CarePlan[], CarePlanError>> {
    try {
      ...
      const carePlanResults = querySnapshot.docs.map((doc) => firestoreDocToCarePlan(doc.id, doc.data()))
      ...
      const carePlans = carePlanResults.map((r) => (r.success ? r.value : null)).filter((c): c is CarePlan => c !== null)
      return Ok(carePlans)
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to find care plans by resident'
      return Err(createUnknownCarePlanError(message))
    }
  }

  async function findActiveByResident(residentId: string): Promise<Result<CarePlan[], CarePlanError>> {
    try {
      const q = query(
        collection(db, collectionName),
        where('residentId', '==', residentId),
      ...
      const message = error instanceof Error ? error.message : 'Failed to find active care plans by resident'
      return Err(createUnknownCarePlanError(message))
    }
  }
  ...
  async function findAll(): Promise<Result<CarePlan[], CarePlanError>> {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to find care plans'
      return Err(createUnknownCarePlanError(message))
    }
  }

  async function update(id: string, updates: Partial<Omit<CarePlan, 'id' | 'createdAt'>>): Promise<Result<CarePlan, CarePlanError>> {
    try {
      const docRef = doc(db, collectionName, id)
      const docSnap = await getDoc(docRef)
  ...
  async function recordActivity(activity: Omit<CarePlanActivity, 'id'>): Promise<Result<CarePlanActivity, CarePlanError>> {
    try {
      const activityData = {
        ...activity,
      ...
      const message = error instanceof Error ? error.message : 'Failed to record care plan activity'
      return Err(createUnknownCarePlanError(message))
    }
  }

  async function getActivityHistory(carePlanId: string): Promise<Result<CarePlanActivity[], CarePlanError>> {
    try {
      const q = query(
      ...
      const message = error instanceof Error ? error.message : 'Failed to get activity history'
      return Err(createUnknownCarePlanError(message))
    }
  }

  async function getResidentActivityHistory(residentId: string): Promise<Result<CarePlanActivity[], CarePlanError>> {
    try {
      const q = query(
        collection(db, activityCollectionName),
        where('residentId', '==', residentId),
      ...
      const message = error instanceof Error ? error.message : 'Failed to get resident activity history'
      return Err(createUnknownCarePlanError(message))
    }
  }
