...
import { Card, Skeleton } from '@design-system/atoms'
...
const { carePlans, isLoading: isLoadingCarePlans } = storeToRefs(carePlanStore)
const { fetchCarePlans } = carePlanStore

const incidentStore = useIncidentStore()
const { incidents, isLoading: isLoadingIncidents } = storeToRefs(incidentStore)
const { fetchIncidents } = incidentStore

const medicationStore = useMedicationStore()
const { medications, isLoading: isLoadingMedications } = storeToRefs(medicationStore)
...
watch(
	...
	residentId => {
		if (residentId) {
			...
			fetchCarePlans(residentId)
			fetchIncidents(residentId)
		}
	},
	{ immediate: true }
)
