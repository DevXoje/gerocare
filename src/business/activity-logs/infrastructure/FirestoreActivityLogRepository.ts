...
} from '@/business/activity-logs/domain/ActivityLogErrors'
 ...
 */
function firestoreDocToActivityLog(
	docId: string,
	...
	try {
		...
		if (!result.success) {
			...
			return Err(
				createActivityLogValidationError(
					`Invalid activity log data from Firestore: ${firstError?.message || 'Validation failed'}`
				)
			)
		}
		...
		const appError = toAppError(error, 'Error al convertir documento de Firestore a ActivityLog')
		logError(appError, { docId, operation: 'firestoreDocToActivityLog' })
		return Err(createActivityLogRepositoryError(appError.message))
	}
}
...
export function createActivityLogRepository(db: Firestore): ActivityLogRepository {
	...
	async function create(
		...
		try {
			...
			const appError = toAppError(error, 'Error al crear registro de actividad')
			logError(appError, { operation: 'create', activityLog })
			return Err(createActivityLogRepositoryError(appError.message))
		}
	}
	...
	async function findById(id: string): Promise<Result<ActivityLog | null, ActivityLogError>> {
		try {
			...
			const appError = toAppError(error, 'Error al buscar registro de actividad')
	...
	async function findByResident(
		...
		try {
			...
			const activityLogResults = querySnapshot.docs.map(doc =>
				firestoreDocToActivityLog(doc.id, doc.data())
			)
			...
			const activityLogs = activityLogResults
				.map(r => (r.success ? r.value : null))
				.filter((c): c is ActivityLog => c !== null)
			return Ok(activityLogs)
		} catch (error) {
			const appError = toAppError(error, 'Error al buscar registros de actividad por residente')
			logError(appError, { operation: 'findByResident', residentId })
			return Err(createActivityLogRepositoryError(appError.message))
		}
	}

	async function findByCaregiver(
		caregiverId: string
	): Promise<Result<ActivityLog[], ActivityLogError>> {
		try {
			const q = query(
			...
			const appError = toAppError(error, 'Error al buscar registros de actividad por cuidador')
	...
	async function findByResidentAndDateRange(
		...
		try {
			...
			const appError = toAppError(
				error,
				'Error al buscar registros de actividad por residente y rango de fechas'
			)
			logError(appError, {
				operation: 'findByResidentAndDateRange',
				residentId,
				startDate,
				endDate,
			})
	...
	async function findAll(): Promise<Result<ActivityLog[], ActivityLogError>> {
		try {
			...
			const appError = toAppError(error, 'Error al buscar todos los registros de actividad')
	...
	async function update(
		...
		try {
			...
			const appError = toAppError(error, 'Error al actualizar registro de actividad')
	...
	async function deleteActivityLog(id: string): Promise<Result<void, ActivityLogError>> {
		try {
			...
			const appError = toAppError(error, 'Error al eliminar registro de actividad')
