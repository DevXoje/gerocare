...
const { form, isLoading, error, isFormValid, submit, resetForm, loadActivityLogToForm } = useActivityLogForm()
    ...
    <form class="activity-log-form" @submit.prevent="handleSubmit">
      ...
      <FormField label="Tipo de Actividad" required>
        <Select :model-value="form.activityType || 'other'"
          @update:model-value="(v) => (form.activityType = v as ActivityLog['activityType'])" :options="activityTypes"
          required />
      </FormField>
      ...
      <FormField label="Descripción" required>
        <Textarea :model-value="form.description || ''" @update:model-value="(v) => (form.description = v)" :rows="3"
          required placeholder="Descripción detallada de la actividad realizada" />
      </FormField>
      ...
      <FormField label="Duración (minutos)" hint="Opcional">
        <Input :model-value="form.duration?.toString() || ''"
          @update:model-value="(v) => (form.duration = typeof v === 'number' ? v : v ? Number(v) : undefined)"
          type="number" min="1" placeholder="Ej: 30" />
      </FormField>

      <FormField label="Estado" required>
        <Select :model-value="form.status || 'completed'"
          @update:model-value="(v) => (form.status = v as ActivityLog['status'])" :options="statuses" required />
      </FormField>

      <FormField label="Notas" hint="Opcional">
        <Textarea :model-value="form.notes || ''" @update:model-value="(v) => (form.notes = v || undefined)" :rows="2"
