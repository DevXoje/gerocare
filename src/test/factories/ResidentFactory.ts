 ...
 * ```
 */
export function createResidentFactory(overrides?: Partial<Resident>): ResidentFactoryBuilder {
	...
	const builder: ResidentFactoryBuilder = {
		...
		withAllergies(...allergies: string[]) {
			...
			resident.medicalInfo.allergies = [...(resident.medicalInfo.allergies || []), ...allergies]
			return builder
		},
		...
		withEmergencyContact(contact: Resident['emergencyContacts'][0]) {
			...
			resident.emergencyContacts = [...resident.emergencyContacts, contact]
			return builder
		},
 ...
 */
export function createManyResidents(count: number, overrides?: Partial<Resident>): Resident[] {
