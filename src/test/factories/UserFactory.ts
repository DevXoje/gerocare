 ...
 * ```
 */
export function createUserFactory(overrides?: Partial<FirebaseUser>): UserFactoryBuilder {
		...
		build(overrides?: Partial<FirebaseUser>): FirebaseUser {
			...
			return {
				...
				metadata: {
					...
					lastSignInTime: user.metadata?.lastSignInTime || Date.now().toString(),
				},
				...
				getIdTokenResult: vi
					.fn()
					.mockResolvedValue({} as Awaited<ReturnType<FirebaseUser['getIdTokenResult']>>),
